...
<template>
  <div v-if="topic" class="min-h-screen bg-neutral-black text-white">
    ...
    <Transition name="slide-down">
      <div v-if="tocOpen" class="lg:hidden fixed inset-0 z-50 flex flex-col" @click.self="closeToc">
        <!-- Backdrop -->
        <div class="absolute inset-0 bg-black/60" @click="closeToc"></div>

        <!-- Drawer -->
        <div
          class="relative mt-auto bg-neutral-black border-t border-neutral-gray max-h-[70vh] overflow-y-auto no-scrollbar"
        >
          <div class="flex items-center justify-between px-4 py-3 border-b border-neutral-gray">
            <h3 class="text-sm font-bold text-primary-lightgreen uppercase tracking-wider">
              Contents
            </h3>
            <button
              @click="closeToc"
        ...
        <div class="flex items-center gap-3 mb-8 pb-6 border-b border-neutral-gray">
          ...
          >
            <img
              :src="topic.icon"
              :alt="topic.label"
              class="w-7 h-7 md:w-9 md:h-9 object-contain"
            />
          </div>
          <div>
